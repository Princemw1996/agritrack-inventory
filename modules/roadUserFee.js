...
async function renderRoadUserFee() {
    ...
    async function renderForm() {
        ...
        let collectorIdentifier = '';
        let userRole = currentUserRole;
        try {
            const { data: { user }, error } = await sb.auth.getUser();
            if (!error && user) {
                collectorIdentifier = user.user_metadata?.name || user.email || '';
            }
        } catch (e) {
            console.warn('Could not get user info:', e);
        }

        // Fetch collections – filter if revenue_collector
        let query = sb.from('road_user_fee_collections').select('*');
        if (userRole === 'revenue_collector' && collectorIdentifier) {
            query = query.eq('collected_by', collectorIdentifier);
        }
        const { data: collections, error } = await query.order('payment_date', { ascending: false });
        if (error) {
        ...
        let html = `
            ...
            <div class="card">
                ...
                <div class="flex-row">
                    ...
                    <div style="flex:1">
                        ...
                        <input type="text" id="feeCollector" value="${collectorIdentifier}" readonly style="background:#f5f5f5;">
                    </div>
                </div>
            ...
            <div class="card">
                <h3>📋 ${userRole === 'revenue_collector' ? 'My Collection History' : 'Collection History'}</h3>
