// modules/mainTransfer.js – with audit logging
        ...
        for (const row of valid) {
            try {
                ...
                await logAction('reduce_stock', 'inventory', inv.id,
                    `Reduced stock at Main Warehouse (location 1) for product ${row.productId} from ${oldQty} to ${newQty} due to transfer`);
                ...
                await logAction('create_transfer', 'transfers', transfer.id,
                    `Transferred ${row.qty} units of product ${row.productId} from Main Warehouse to Min Warehouse`);
                success++;
            } catch (err) {
