async function renderMinReceive() {
    ...
    console.log('Fetching pending transfers for Min Warehouse (location_id=2)');
    ...
    document.querySelectorAll('.confirmReceiveMin').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            if (!t) return;
            // Add to Min Warehouse inventory (location_id=2)
            ...
            if (inv) {
                ...
                await sb.from('inventory')
                    .insert({ location_id: 2, product_id: t.product_id, quantity: t.quantity, last_updated: new Date().toISOString() });
            }
